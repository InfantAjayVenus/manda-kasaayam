...
const MarkdownPreview: React.FC<MarkdownPreviewProps> = ({ content, title, onExit }) => {
  ...
  const highlightCode = useCallback((code: string, language?: string): React.ReactNode => {
    ...
    return lines.map((line, lineIndex) => {
        ...
        if (keywords || strings || comments) {
          ...
          if (keywords) {
            keywords.forEach(keyword => {
              ...
              segments.push(<Text key={index} color="blueBright">{keyword}</Text>);
      ...
      return (
        <Box key={lineIndex}>
          <Text color="gray" dimColor>{String(lineIndex + 1).padStart(2, ' ')} </Text>
          <Text>{segments}</Text>
        </Box>
      );
    });
    ...
    switch (token.type) {
      case 'heading':
       ...
       case 'paragraph':
         return (
           <Box key={index} marginBottom={1}>
             {token.tokens?.map((childToken, childIndex) => renderToken(childToken, childIndex)) || <Text>{token.text}</Text>}
           </Box>
         );
          ...
          <Box key={index} marginBottom={1} borderStyle="round" borderColor="gray">
            {token.lang && (
              <Box backgroundColor="gray" paddingX={1}>
                <Text color="white" bold>{token.lang}</Text>
              </Box>
            )}
      ...
      case 'codespan':
        return <Text key={index} backgroundColor="gray" color="green">`{token.text}`</Text>;

      case 'blockquote':
        return (
          <Box key={index} marginBottom={1} paddingLeft={2} borderStyle="single" borderLeft borderColor="cyan">
      ...
      case 'strong':
        return (
          <Text key={index} bold color="white">
            {token.tokens?.map((childToken, childIndex) => renderToken(childToken, childIndex)) || token.text}
          </Text>
        );
      ...
      case 'link':
       ...
       case 'table':
         ...
         if (tableToken.header && Array.isArray(tableToken.header)) {
           // Header row
           tableText += tableToken.header.map((h: any) => typeof h === 'string' ? h : h.text || h).join(' | ') + '\n';
           // Separator
